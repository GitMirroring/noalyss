...
class Lettering
{
    ...
    /**
     * @brief Use to just insert a couple of lettered operation but do not make a link between operation
     */
    function insert_couple($j_id1, $j_id2)
    {
        ...
        if ($first=='t')
        {
            ...
            if ($let1==0)
                $ld_id=$this->db->get_value('insert into letter_deb(j_id,jl_id) values($1,$2) returning ld_id',
                        array($j_id1, $jl_id));
        }
        else
        {
            if ($let1==0)
                $lc_id=$this->db->get_value('insert into letter_cred(j_id,jl_id)  values($1,$2) returning lc_id',
                        array($j_id1, $jl_id));
        }
        if ($second=='t')
        {
    ...
    /**
     * @brief save from array, letter the accounting (or card) and create a link between operation
     * 
     * @param $p_array
      @code
    ...
    public function save($p_array)
    {
        ...
        $nb_letter_j_id=count($p_array['letter_j_id']);
        ...
        try
        {
            ...
            // save dest
            for ($i=0; $i<$nb_letter_j_id; $i++)
            {
                if (! isset($p_array['ck'][$i]) || $p_array['ck'][$i] == "-2" ) { continue ; }
                
                // save the dest
                $deb=$this->db->get_value('select j_debit,j_montant from jrnx where j_id=$1',
                        array($p_array['ck'][$i]));
                if ($deb=='t')
                {
                    // save into letter_deb
                    $ld_id=$this->db->get_value('insert into letter_deb(j_id,jl_id) values($1,$2) returning ld_id',
                            array($p_array['ck'][$i], $jl_id));
                }
                else
                {
                    $lc_id=$this->db->get_value('insert into letter_cred(j_id,jl_id)  values($1,$2) returning lc_id',
                ...
                $acc_reconciliation_lettering=new Acc_Reconciliation_Lettering($this->db);
                $acc_reconciliation_lettering->insert_reconcilied($p_array['ck'][$i], $p_array['j_id']);
            } //end for
